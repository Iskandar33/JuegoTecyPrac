...
typedef struct nodoArbol{
     int valoridA, idN;
     struct nodoArbol* pHijoI;
     struct nodoArbol* pHijoD;    
}nodoArbol;
...
int  leerArchivo();
//Arbol
nodoArbol* crearNodoArbol(int idA, int idN);

Nodo* crearNodoLista(int numCarta);
...
void crearArBinEscribir(){
    ...
    if(archivo == NULL){
        ...
        fwrite(&jugador.oro, sizeof(int), 1, archivo);
        fwrite(&jugador.vida, sizeof(int), 1, archivo);
        fwrite(jugador.enemigos, sizeof(int) * ENEMIGOSIZE, 1, archivo);
        fwrite(jugador.deck, sizeof(int) * CARTASIZE, 1, archivo);
        fwrite(&jugador.opcionPsj, sizeof(int), 1, archivo);
        fwrite(&jugador.posActual, sizeof(int), 1, archivo);
        fclose(archivo);
    }
    printf("\nGuardado exitoso\n");
}
...
int leerArchivo(){
    ...
    if(archivo == NULL){
        ...
        fread(&jugador.oro, sizeof(int), 1, archivo);
        fread(&jugador.vida, sizeof(int), 1, archivo);
        for(int i = 0; i < ENEMIGOSIZE; i++){
            fread(&jugador.enemigos[i], sizeof(int), 1, archivo);
        }
        for(int i = 0; i < CARTASIZE; i++){
            fread(&jugador.deck[i], sizeof(int), 1, archivo);
        }
        fread(&jugador.opcionPsj, sizeof(int), 1, archivo);
        // printf("Posición nodo: %d\n", jugador.posActual);
        fread(&jugador.posActual, sizeof(int), 1, archivo);
    }
    jugador.armadura = 0;
    fclose(archivo);
    return 1;
...
nodoArbol* crearNodoArbol(int idA, int idN){
    nodoArbol* nuevoN = (nodoArbol*)malloc(sizeof(nuevoN));
    nuevoN->valoridA = idA;
    nuevoN->idN = idN; 
    nuevoN->pHijoD = NULL;
    nuevoN->pHijoI = NULL;
...
void crearArbol(nodoArbol*arr[16]){
    srand(time(NULL));
    nodoArbol* nuevo;
    int ran;
    for(int i = 0; i < 16; ++i){
        ran = rand() % 2 + 1;
        if(i == 0){
            nuevo = crearNodoArbol(0,i);
            arr[i] = nuevo;
        }
        else if(i < 9){
            if(i == 8){
                nuevo = crearNodoArbol(3,i);
                arr[i] = nuevo;
            }
            else{
                nuevo = crearNodoArbol(ran,i);
                arr[i] = nuevo;
            }
        }
        else{
            if(i <= 12){
                nuevo = crearNodoArbol(4,i);
                arr[i] = nuevo;
            }
            else{
                nuevo = crearNodoArbol(5,i);
                arr[i] = nuevo;
            }
        }
    }

    arr[0]->pHijoD = arr[1]; arr[0]->pHijoI = arr[2];
    arr[1]->pHijoD = arr[3]; arr[1]->pHijoI = arr[13];
    arr[2]->pHijoD = arr[9]; arr[2]->pHijoI = arr[4];
    arr[3]->pHijoD = arr[14]; arr[3]->pHijoI = arr[10];
    arr[4]->pHijoD = arr[12]; arr[4]->pHijoI = arr[7];
    arr[5]->pHijoD = arr[8]; arr[5]->pHijoI = arr[8];
    arr[6]->pHijoD = arr[8]; arr[6]->pHijoI = arr[8];
    arr[7]->pHijoD = arr[8]; arr[7]->pHijoI = arr[8];
    arr[9]->pHijoD = arr[15]; arr[9]->pHijoI = arr[6];
    arr[10]->pHijoD = arr[8]; arr[10]->pHijoI = arr[8];
    arr[11]->pHijoD = arr[8]; arr[11]->pHijoI = arr[8];
    arr[12]->pHijoD = arr[8]; arr[12]->pHijoI = arr[8];
    arr[13]->pHijoD = arr[5]; arr[13]->pHijoI = arr[11];
    arr[14]->pHijoD = arr[8]; arr[14]->pHijoI = arr[8];
    arr[15]->pHijoD = arr[8]; arr[15]->pHijoI = arr[8];
}


int menuInicial(){
    int opcPar, PartidaA, banopcP = 0;
    PartidaA = leerArchivo();
    if(PartidaA == 0){
        printf("\n    Hexed Abism   \n");
        printf("\n  >>>> Menu <<<<\n  1. Nueva Partida\n  2. Salir\n\n");
        while(banopcP == 0){
...
void caminar(){
    int opc = 0;
    nodoArbol* nodoActual = arbol[jugador.posActual];
    printf("Aventurero elige uno de los Caminos\n");
    printf("1. Izquierda = ");
    if(nodoActual->pHijoI->valoridA == 5){
    	printf("Tienda\n");
    	
    }
    else if(nodoActual->pHijoI->valoridA == 4){
    ...
    printf("2. Derecha = ");
    if(nodoActual->pHijoD->valoridA == 5){
    	printf("Tienda\n");
    	
    }
    else if(nodoActual->pHijoD->valoridA == 4){
    	printf("Zona de Descanso\n");
    ...
    while(opc != 1 && opc != 2){
        printf("Ingrese su decisión: ");
        scanf("%d",&opc);
    }
    if(opc == 1){
    	nodoActual = nodoActual->pHijoI;
        jugador.posActual = nodoActual->idN;
    	if((nodoActual->valoridA == 1) || (nodoActual->valoridA == 2) || (nodoActual->valoridA == 3)){
            luchar(nodoActual->valoridA);
        }
        else if(nodoActual->valoridA == 4){
            if(jugador.vida + 20 > arregloPersonajes[jugador.opcionPsj].vidaTotal){
                jugador.vida = arregloPersonajes[jugador.opcionPsj].vidaTotal;
            }
            else{
                jugador.vida += 20;
            }
		...
		if((nodoActual->valoridA == 1) || (nodoActual->valoridA == 2) || (nodoActual->valoridA == 3)){
            ...
            if(jugador.vida + 20 > arregloPersonajes[jugador.opcionPsj].vidaTotal){
                jugador.vida = arregloPersonajes[jugador.opcionPsj].vidaTotal;
                printf("Te has curado completamente, vida actual: %d/%d\n", jugador.vida, arregloPersonajes[jugador.opcionPsj].vidaTotal);
            }
            else{
                jugador.vida += 20;
                printf("Te has curado 20 de vida, vida actual: %d/%d\n", jugador.vida, arregloPersonajes[jugador.opcionPsj].vidaTotal);
            }
        }
        else{
            tienda();
...
void añadirCarta(Lista* listaCartas, int pCarta){
    ...
    Nodo* nuevaCarta;
    int flag = 0;
    nuevaCarta = crearNodoLista(pCarta);
...
int utilizarCarta(Lista* miniDeck, int posicion, int hpEnemigo){
    ...
    if(arrayCartas[nodoAux->numCarta].categoria == 2){
        ...
        if(arregloPersonajes[jugador.opcionPsj].fuerza > 1){
            dañoTotal = 1.5 * (arrayCartas[nodoAux->numCarta].num);
            hpEnemigo -= dañoTotal;
        ...
        if(arregloPersonajes[jugador.opcionPsj].dureza > 1){
            armaduraTotal = 1.5 * (arrayCartas[nodoAux->numCarta].num);
...
int menuLuchar(Lista* miniDeck, int energia){
    ...
    printf("\n\n Ingrese una opción: ");
    scanf("%d", &opcion);
    while((opcion > 6) || (opcion > miniDeck->tamaño - 1)){
        // printf("Tamaño: %d\n", miniDeck->tamaño);
        // printf("Tamaño2: %d\n", miniDeck->tamaño - 1);
        printf("Ingrese una opción válida\n");
        printf("\n\n Ingrese una opción: ");
    ...
    while((arrayCartas[nodoAux->numCarta].costoEnergia > energia) && (flag != 1)){ 
...
void luchar(int tipoLucha){
    int oroGanado, numeroEnemigo, hpEn, atEn, dañoEnemigo, cancelarTurno, accionEnemigo, accion, buffAtaque, gastoEnergia, energia = 5;
    Lista* miniDeck = crearLista();
    srand(time(NULL));
    if(tipoLucha == 1){
        numeroEnemigo = rand() % 15;
        printf("Ingresara en una lucha normal...\n");
    }
    else if(tipoLucha == 2){
        numeroEnemigo = rand() % 10 + 15;
        printf("Ingresara en una lucha elite...\n");
    }
    else{
        numeroEnemigo = rand() % 5 + 25;
        printf("Ingresara en una contra el Jefe Final!...\n");
    }
    ...
    if(hpEn <= 0){
        ...
        if(jugador.posActual != 8){
            encontrarCarta();
            oroGanado = rand() % 35 + 1;
            jugador.oro += oroGanado;
            printf("Después del combate, has encontrado %d de oro\n", oroGanado);
            printf("Tu oro actual es de %d\n", jugador.oro);
        }
...
void imprimirDeck(){
    for(int i = 0; i < CARTASIZE; i++){
        if(jugador.deck[i] != 0){
            printf("Carta\n Nombre: %s\n Tipo: %d\n Costo de energia: %d\n Cantidad: %d\n Efecto: %s\n", 
                arrayCartas[i].nombre,arrayCartas[i].categoria,
                    arrayCartas[i].costoEnergia, jugador.deck[i], arrayCartas[i].efecto);
        }
    }
}
...
void menuJuego(){
    ...
    do{
        ...
        switch(opc){
            ...
            case 2: imprimirDeck();
            break;
            case 3: imprimirBestiario();
            break;
            case 4: crearArBinEscribir();
                    printf("\n Juego Guardado...\n");
            break;
            case 5: printf("Nos volveremos a encontrar en el abismo...\n");
            break;
            default:
            break;
        }
    }while((opc!=5) && (jugador.vida > 0) && (jugador.posActual != 8));
    if((jugador.posActual == 8) && (jugador.vida > 0)){
        printf("\n\n         ¡Felicidades!   \n   Lograste escapar del abismo...\n");
    }
}
...
void juego(){
    ...
    while(banJuego == 0){
    	...
    	if(opcMP == 1){
            ...
            printf("\n Juego cargado...\n");
            printf("Posición actual: %d", jugador.posActual);
            menuJuego();
   		}
        else{
   			banJuego = 1;
